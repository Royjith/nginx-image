pipeline {
    ...
    environment {
        ...
        DOCKER_TAG = 'version-v018'             // Docker tag
        ...
        DEPLOYMENT_NAME = 'pipeline-deployment1'
    ...
    stages {
        ...
        stage('Build Docker Image') {
            steps {
                script {
                    // Use the static Docker tag "version-v018"
                    def tag = "${DOCKER_TAG}"
                    ...
                    try {
                        ...
                        sh "docker build -f Dockerfile --no-cache -t ${DOCKER_HUB_REPO}:version-v018 ."

                        // Tag the image with the appropriate repository and tag
                        echo "Tagging the Docker image with tag: ${DOCKER_HUB_REPO}:${tag}..."
                        sh """
                            IMAGE_ID=\$(docker images -q ${DOCKER_HUB_REPO}:version-v018)
